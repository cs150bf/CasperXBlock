...
order_even = compute_order(map_even);
fmtstr = sprintf('order=%d', order_even);
...
order_odd = compute_order(map_odd);
fmtstr = sprintf('order=%d', order_odd);
set_param(gcb, 'AttributesFormatString', fmtstr);
...
if (sync_delay > 52),
    ...
    xBlock( struct('name',  sync_delay_name, 'source', str2func('delay_slr_init_xblock')),  ...
    { sync_delay}, ... 	
    {reorder_even_sync_out}, {sync_delay_1});
end
...
if strcmp(dsp48_adders, 'on'),
    ...
    {n_bits, add_latency, conv_latency}, ... 	
     {Mux0_1, Mux1_1}, {hilbert_0_1, hilbert_0_2});
...
order_out = compute_order(map_out);
fmtstr = sprintf('order=%d', order_out);
set_param(gcb, 'AttributesFormatString', fmtstr);
